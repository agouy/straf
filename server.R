...
# GET INPUT FILE -------------------------------------------------------------
  ...
  getData <- reactive({ #get file from the input
    ...
    if(is.null(input$file1)) {
      ...
      X <- read.table(input$file1$datapath, header = TRUE,
                    sep = "\t",colClasses="character")

      if(class(X) != "data.frame") {warning("Input file error."); return(NULL)}
      if(colnames(X)[1] != "ind") {warning("Input file error."); return(NULL)}
      if(colnames(X)[2] != "pop") {warning("Input file error."); return(NULL)}
      if(dim(X)[1]<2) {warning("Input file error."); return(NULL)}
      if(dim(X)[2]<3) {warning("Input file error."); return(NULL)}
      
      testGeno <- try(createGenind(Ifile=input$file1,
                             Imicrovariants=input$microvariants,
                             Incode=input$ncode,
                             Iploidy=input$ploidy), silent=TRUE)
      
      if(class(testGeno)=="try-error") {warning("Input file error."); return(NULL)}
      
      testGeno2 <- try(genind4LD(Ifile=input$file1,Imicrovariants=input$microvariants,
                      Incode=input$ncode,Iploidy=input$ploidy), silent=TRUE)
      
      if(class(testGeno2)=="try-error") {warning("Input file error."); return(NULL)}
      
      if(length(unique(testGeno2@pop)) > 1) {
        
        testGeno3 <- try(wc(testGeno2,diploid = switch(
          input$ploidy, Diploid = TRUE, Haploid = FALSE)), silent=TRUE)
        
        if(class(testGeno3)=="try-error") {
          warning("Input file error."); return(NULL)
        }
  ...
  output$checkInputFile <- renderText({ ### print an error message in the main panel
    if(is.null(input$file1)) {
      ...
      if(class(X) != "data.frame") return("Input file is not a data frame. Please read the documentation for more information.")
      if(colnames(X)[1] != "ind") return("Input file error. Please read the documentation for more information.")
      if(colnames(X)[2] != "pop") return("Input file error. Please read the documentation for more information.")
      if(dim(X)[1]<2) return("Input file error: incorrect number of rows. Please read the documentation for more information.")
      if(dim(X)[2]<3) return("Input file error: incorrect number of columns. Please read the documentation for more information.")
      ...
      if(class(testGeno)=="try-error") {return("Input file error. Wrong number of
                                               columns per locus (please check the left tab).")}
      
      testGeno2 <- try(genind4LD(Ifile=input$file1,
                                 Imicrovariants=input$microvariants,
                                 Incode=input$ncode,
                                 Iploidy=input$ploidy),
                       silent=TRUE)
...
# ALLELE FREQUENCIES -----------------------------------------------------------
  ...
  output$alleleFreq <- renderPlot({ #barplots of allele freq distributions
    ...
    dat2 <- createGenind(Ifile=getData(),
                         Imicrovariants=input$microvariants,
                         Incode=input$ncode,
                         Iploidy=input$ploidy)
    ...
    for(i in unique(DAT$loc)) {
      ...
      barplot(DAT$freq[DAT$loc==i],
              names.arg=DAT$alle[DAT$loc==i],
              main=i,
              col=transp(input$barplotcolor,
                         input$transparency),
...
# STATISTICS TABLE -------------------------------------------------------------
  ...
  reacIndices <- reactive({
    ...
    DF <- getIndicesAllPop(dat2, hw = hw, ploidy = ploidy)
    return(DF)
  ...
  output$forensics <- renderTable({
    ...
    if(!is.null(input$selectPop2)) {
      if(length(unique(dat2@pop))>1 & input$selectPop2 == "all") {
        taB[,-which(colnames(taB) %in% c("Ht","Fis","Fst"))]
      } else {
        taB
      }
    } else {
  ...
  output$dlForensics <- downloadHandler(
    ...
    content = function(file) {
      ...
      write.table(taB[,-which(colnames(taB) %in% c("Ht","Fis","Fst"))], 
  ...
  output$diversity <- renderTable({
    ...
    taB2 <- taB[,-which(colnames(taB) %in% c("PIC","PD","PE","TPI", "PM"))]
    taB2
  ...
  output$plotIndices <-  renderPlot({
    ...
    barplot(dat[,input$plotIndicesFOR],
            names.arg = dat[,"locus"],
            horiz=TRUE,
            las = 1,
  ...
  output$plotIndicesPopgen <-  renderPlot({
    ...
    par(mar=rep(input$margin,4))
    barplot(datpl[,input$plotIndicesPG],
            names.arg = datpl[,"locus"],horiz=TRUE,
            las = 1,
            col = transp(input$barplotcolor,input$transparency),
            border = as.numeric(input$borderbarplot),
            cex.axis = input$cexaxis, 
...
# LD ---------------------------------------------------------------------------
  ...
  output$plotLD2 <- renderUI({
    plotOutput('plotLD', width=paste(input$width,"%",sep=""), 
               height=input$height)
  })
  ...
  output$plotLDpval2 <- renderUI({
    plotOutput('plotLDpval', width=paste(input$width,"%",sep=""), 
               height=input$height)
  })
...
getIndicesAllPop <- function(data, hw=FALSE, ploidy="Diploid") {
  
  ind <- list()
  ind$all <- getIndicesFromGenind(data, hw, ploidy)
  
  for(popu in unique(data$pop)) {
    ind <- c(ind,x=NA)
    mat <- getIndicesFromGenind(data[data@pop==popu,], hw, ploidy)
    ind$x <- mat
...
getIndicesFromGenind <- function(data, hw=FALSE, ploidy="Diploid") {
  ...
  if(length(unique(data@pop)) > 1) {
    ...
    basicstat <- basic.stats(data, diploid = switch(
      ploidy, Diploid = TRUE, Haploid = FALSE), digits = 4)
    
    Fst <- wc(data,diploid = switch(
      ploidy, Diploid = TRUE, Haploid = FALSE))$per.loc$FST
    ...
    if(ploidy=="Diploid" & hw) {
      DF$pHW <- hw.test(data,B=1000)[names(GD),4]
    } 
